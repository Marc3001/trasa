...
func (s authStore) GetLoginDetails(trasaID, orgDomain string) (*models.UserWithPass, error) {
	...
	if isTrasaIDEmail {
		sqlStr = `SELECT users.org_id, users.id, username, first_name, last_name, email, password, idp_name, user_role, status ,org.org_name
				FROM users
...
`
	...
	err := s.DB.QueryRow(sqlStr, trasaID).Scan(&user.OrgID, &user.ID, &user.UserName, &user.FirstName, &user.LastName, &user.Email, &user.Password, &user.IdpName, &user.UserRole, &user.Status, &user.OrgName)
